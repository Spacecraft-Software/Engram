...
- `consolidate --extract [--scope <id>] [--dry-run]` — idle-time maintenance (M4): run the deterministic fact extractor over the current, non-rule memories and upsert into the facts index. Without `--extract` it is an InvalidArgument (exit 2; dedup/decay arrive at M5). `--scope` omitted means **every** scope — no cascade, unlike the rule commands. CLI-only by design
