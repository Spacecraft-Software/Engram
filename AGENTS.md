...
## The harness table, and what it may claim

`src/harness.rs`. 21 entries, all present on the author's machine. The table's failure mode is not omission but **false confidence** — three entries once told users a thing was impossible when it was not, and nobody re-checks a documented impossibility:

| Harness | Once claimed | Verified truth |
|---|---|---|
| Antigravity | "protocol buffers … no line-oriented transcript to read" | writes plain JSONL at `antigravity-cli/brain/<uuid>/.system_generated/logs/transcript.jsonl` — **54 of them here** (`fd` hides it: `.system_generated` is a dot-directory, so the count is 0 without `-H`) |
| Copilot CLI | "SQLite database with an undocumented schema" | `turns(session_id, turn_index, user_message, assistant_response, timestamp)`, `UNIQUE(session_id, turn_index)`, joined to `sessions(cwd, repository, branch)` — a flat pre-paired transcript |
| Kimi | "the project directory is a hash with no published mapping back to a working directory" | reversible three ways: `session_index.jsonl` maps session→workDir, each `state.json` carries `workDir`, and the legacy hash is **MD5 of the absolute path** (verified against two directories) |
...
- **Grok's cwd mapping is reversible**, unlike Claude Code's: `.grok/sessions/%2Fspacecraft-software%2Fbravais` is the percent-encoded absolute path, so a reader would not need the cwd supplied.
- **A layout that resembles Claude Code's is not Claude Code's.** `deepcode` writes `projects/<mangled-cwd>/<uuid>.jsonl`, but its records carry none of the keys (`type`, `uuid`, `cwd`) the reader dispatches on — the resemblance is the directory naming only.
- **ZCode is an opencode fork** (`.zcode/cli/db/db.sqlite`, same `session`/`message`/`part` schema), so one reader would serve both, exactly as `claude_code` serves OpenClaude. Its sibling `rollout/model-io-*.jsonl` is an HTTP log that resends the entire conversation on every line — forensics, not a transcript.
...
- **Grok needs no command *surface*, and writing one would duplicate its commands.** (It does have a table entry now — recording what it stores is not the same as writing to it.)
  ...
  `~/.cursor/skills/` and `.agents/`. Verified with `grok inspect`, which lists
  `engram-context`, `engram-ingest` and `engram-save-chat` as `user [claude]`
  alongside `engram (stdio)` under MCP Servers --- engram's Claude Code install
  already serves Grok. Its own `~/.grok/skills` is a home-manager symlink into
  the store here anyway. Grok dedups by name, so a second copy would be
  shadowed rather than useful.
- **Copilot CLI takes plugins, not files.** `copilot plugin install` accepts a
  ...
  can drop a command into. The `~/.copilot/skills` symlink on this machine
  belongs to the user's own skills bundle, not to Copilot --- do not read it as
  evidence of a writable surface.
