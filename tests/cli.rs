...
/// An `engram import` invocation against a project directory.
fn import(db: &Path, project: &Path, args: &[&str]) -> Command {
    let mut cmd = engram(db);
    cmd.current_dir(project);
    cmd.arg("import");
    cmd.args(args);
...
/// Writes a small engram archive into `<project>/chat/`.
fn plant_archive(project: &Path, name: &str, body: &str) -> std::path::PathBuf {
    let dir = project.join("chat");
...
const ARCHIVE: &str = "\\input texinfo @c -*-texinfo-*-\n\
...
/// Round-trips what `save-chat` writes, which engram could not read back.
///
/// The scope comes from the directory the archive sits in, not from the
/// `@settitle` line — recorded scopes in the real corpus are inconsistent
/// (bare names, paths, absolute paths, branch names), so the containing
/// project is the reliable signal.
#[test]
fn import_reads_an_engram_archive_and_is_idempotent() {
    let tmp = TempDir::new().expect("tempdir");
    let db = tmp.path().join("test.db");
    let project = pinned_project(&tmp);
    plant_archive(&project, "2026-07-26T00-00-00Z.texi", ARCHIVE);

    let assert = import(&db, &project, &["chat"]).assert().success();
    let data = parse_single_line_json(&assert.get_output().stdout)["data"].clone();
    assert_eq!(data["inserted"], 3);
    assert_eq!(
        data["scopes"][0], "project",
        "scope is the containing project"
    );
    assert_eq!(data["files"][0]["format"], "engram-texinfo");
    assert_eq!(data["files"][0]["approximate_times"], false);

    // The third role survives rather than being coerced.
    let assert = engram(&db)
        .args(["recall", "--scope", "project", "--limit", "10"])
        .assert()
        .success();
    let roles: Vec<String> = parse_single_line_json(&assert.get_output().stdout)["data"]
        .as_array()
        .expect("memories")
        .iter()
        .map(|m| m["role"].as_str().unwrap_or_default().to_string())
        .collect();
    assert!(roles.contains(&"note".to_string()), "roles: {roles:?}");
    assert!(roles.contains(&"user".to_string()));
    assert!(roles.contains(&"assistant".to_string()));

    // Identity is the content, so a second run writes nothing.
    let assert = import(&db, &project, &["chat"]).assert().success();
    let data = parse_single_line_json(&assert.get_output().stdout)["data"].clone();
    assert_eq!(data["inserted"], 0);
    assert_eq!(data["skipped_existing"], 3);
}

/// A file that is not a transcript is skipped with a reason, not an error.
///
/// A `chat/` directory accumulates whatever was dropped in it — the real one
/// held a standalone HTML palette editor alongside real archives. Sniffing
/// content rather than extension is what keeps that out, and reporting the
/// skip is what distinguishes "found nothing" from "could not read nine
/// files".
#[test]
fn import_skips_what_it_cannot_recognise_and_says_so() {
    let tmp = TempDir::new().expect("tempdir");
    let db = tmp.path().join("test.db");
    let project = pinned_project(&tmp);
    plant_archive(&project, "real.texi", ARCHIVE);
    plant_archive(
        &project,
        "palette.html",
        "<!doctype html>\n<title>Palette Lab</title>\n<style>:root{}</style>\n",
    );
    ...
    let skipped = data["skipped"].as_array().expect("skipped");
    ...
    assert!(
        skipped[0]["reason"]
            .as_str()
            .expect("reason")
            .contains("no known export format"),
        "{skipped:?}"
    );
}

/// One run over several projects files each into its own scope.
#[test]
fn import_files_each_project_into_its_own_scope() {
    let tmp = TempDir::new().expect("tempdir");
    let db = tmp.path().join("test.db");
    ...
    for name in ["alpha", "beta"] {
        ...
        std::fs::write(project.join(".git"), "gitdir: elsewhere\n").expect("pin git root");
        plant_archive(&project, "a.texi", ARCHIVE);
    }

    let here = pinned_project(&tmp);
    let alpha = tmp.path().join("alpha").to_string_lossy().into_owned();
    let beta = tmp.path().join("beta").to_string_lossy().into_owned();
    let assert = import(&db, &here, &[&alpha, &beta, "--recursive"])
        .assert()
        .success();
    let data = parse_single_line_json(&assert.get_output().stdout)["data"].clone();

    let mut scopes: Vec<String> = data["scopes"]
        .as_array()
        .expect("scopes")
        .iter()
        ...
        .collect();
    scopes.sort();
    assert_eq!(scopes, vec!["alpha".to_string(), "beta".to_string()]);
}

/// Timestamp-less exports are ordered, flagged, and still importable.
#[test]
fn import_synthesises_ordered_times_for_a_markdown_export() {
    let tmp = TempDir::new().expect("tempdir");
    let db = tmp.path().join("test.db");
    let project = pinned_project(&tmp);
    plant_archive(
    ...
    let assert = import(&db, &project, &["chat"]).assert().success();
    let data = parse_single_line_json(&assert.get_output().stdout)["data"].clone();
    assert_eq!(data["files"][0]["format"], "opencode-markdown");
    assert_eq!(data["files"][0]["approximate_times"], true);
    assert_eq!(
        data["inserted"], 2,
        "a heading inside a reply is not a turn"
    );

    // Order is exact even though the absolute times are not.
    let assert = engram(&db)
        .args(["recall", "--scope", "project"])
        .assert()
        .success();
    let mems = parse_single_line_json(&assert.get_output().stdout)["data"].clone();
    let times: Vec<&str> = mems
        .as_array()
        .expect("memories")
        .iter()
